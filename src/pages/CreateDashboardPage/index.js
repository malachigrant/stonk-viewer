  ...
  return (
    <Card>
      ...
      {lists.map((listData, i) => {
        return (
          <Card centered={false} key={i}>
            <Textbox
              label={'List Name'}
              value={listData.name}
              onChange={(name) => updateListData({ name }, i)}
            />
            <ListCreator
              initialList={listData.symbols}
              elementProcessor={textProcessor}
              listChange={(list) => updateListData({ symbols: list }, i)}
            />
          </Card>
        );
      })}
      ...
      <ButtonRow>
        <Button
          type={'confirm'}
          text={dashboardName ? 'Save' : 'Create'}
        ...
        <Button
          ...
          text={'Cancel'}
          onClick={() =>
            setRedirect(dashboardName ? `/dashboard/${dashboardName}` : '/')
          }
        />
      </ButtonRow>
